...
// 更新数据
void handles::update(const CThostFtdcDepthMarketDataField *data)
{
...
// 增加处理器
void handles::add_handler(handler_base *handler)
{
    if (_handles.find(handler->name()) == _handles.end())
    {
        reporter.report(std::string("添加处理器: ") + handler->name());
        _handles[handler->name()] = handler;
    }
}

// 删除处理器
void handles::remove_handler(std::string handle_name)
{
    if (_handles.find(handle_name) != _handles.end())
    {
        reporter.report(std::string("删除处理器: ") + handle_name);
        _handles.erase(handle_name);
    }
}
